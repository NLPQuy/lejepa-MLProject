...
Học $f_\theta : \mathbb{R}^D \to \mathbb{R}^K$ sao cho embedding \textbf{có ích cho mọi downstream task} — không cần nhãn trong lúc train.
...
\vspace{0.3em}
\begin{primarybox}
\footnotesize \textbf{Foundation model:} một hệ thống giải được nhiều downstream task sau khi train một lần, không thay đổi tham số $\theta$.
\end{primarybox}
...
\begin{tikzpicture}[scale=1.05, every node/.style={font=\small}]
  ...
  \draw[decorate, decoration={brace, mirror, amplitude=6pt}, mbad!55, thick]
    ...
    node[midway, below=6pt, font=\scriptsize, mbad]
      {empirical patches — không có first principles};

\end{tikzpicture}
\end{center}
\vfill
\takeaway{Câu hỏi sai: \emph{"tránh collapse thế nào?"} $\;\longrightarrow\;$ Câu hỏi đúng: \emph{"distribution nào là tối ưu?"}}
...
\begin{frame}{Distribution Nào Là Tối Ưu Cho Foundation Model?}
...
\begin{primarybox}
\footnotesize \textbf{Lemma 1 (Bias):} Với $\lambda>0$, luôn tồn tại downstream task mà $\mathbf{Z}_{\rm aniso}$ cho bias \emph{cao hơn} $\mathbf{Z}_{\rm iso}$.
\end{primarybox}

\vspace{0.45em}
\begin{primarybox}
\footnotesize \textbf{Lemma 2 (Variance):}
$\mathrm{tr}(\mathrm{Var}(\hat\beta_{\rm aniso})) > \mathrm{tr}(\mathrm{Var}(\hat\beta_{\rm iso}))$
\end{primarybox}

\vspace{0.45em}
Kết quả tương tự với kNN và kernel regression (Theorem 1 trong paper).

\vspace{0.5em}
\begin{accentbox}
\footnotesize \textbf{Theorem 1:} $\mathcal{N}(\mathbf{0}, I)$ là \textbf{unique minimizer} của downstream risk — với mọi task, mọi probe family.
\end{accentbox}
...
\begin{frame}{Lemma 1 — Anisotropy Làm Tăng Bias}
...
\vspace{0.4em}
\begin{primarybox}
\footnotesize\textbf{Lemma 1.} Với $\lambda>0$, $\exists\,\mathbf{y}$ sao cho:
\[
...
\textbf{Tại sao?} Ridge estimator bị co tất cả các hướng theo $(\Lambda + \lambda I)^{-1}$. Hướng nào có eigenvalue $\lambda_k$ nhỏ → bị co mạnh hơn → bias lớn hơn.
...
\begin{frame}{Lemma 2 — Anisotropy Làm Tăng Variance}
...
\vspace{0.4em}
\begin{primarybox}
\footnotesize\textbf{Lemma 2.} Với $\lambda=0$:
\[
  \mathrm{tr}(\mathrm{Var}(\hat\beta_{\rm aniso})) > \mathrm{tr}(\mathrm{Var}(\hat\beta_{\rm iso}))
\]
Đẳng thức $\Leftrightarrow$ mọi eigenvalue bằng nhau.
\end{primarybox}

\column{0.48\textwidth}
\vspace{0.5em}

\textbf{Minh họa:} Mỗi lần resample training data cho ra một decision boundary khác nhau. Anisotropic embedding $\Rightarrow$ boundaries dao động nhiều hơn, isotropic thì ổn định.
...
\begin{frame}{Theorem 1 — Isotropic Gaussian Là Unique Minimizer}
...
\begin{accentbox}
\footnotesize\textbf{Theorem 1} (Balestriero \& LeCun, 2025). Trong số mọi phân phối $p$ với $\mathrm{Tr}(\mathrm{Cov})=\kappa$, unique minimizer của ISB là:
\[
...
\begin{goodbox}
\footnotesize\textbf{Unique:} không distribution nào khác — kể cả Student-$t$, Laplace, Uniform — đạt được minimum này.\\[4pt]
...
\vfill
\begin{center}
  {\footnotesize\color{mgray}\itshape Theorem 1 (Balestriero \& LeCun, 2025) implying:}\\[14pt]
...
\column{0.32\textwidth}
  ...
  \textcolor{mgood}{\textbf{\checkmark}}\;\textbf{Linear probe}\\optimal (bias + var)
\column{0.32\textwidth}
  \centering\footnotesize
  \textcolor{mgood}{\textbf{\checkmark}}\;\textbf{kNN probe}\\optimal (ISB)
\column{0.32\textwidth}
  \centering\footnotesize
  \textcolor{mgood}{\textbf{\checkmark}}\;\textbf{Kernel probe}\\optimal (ISB)
\end{columns}
\vfill
...
\begin{itemize}\setlength\itemsep{5pt}
  \item[\textcolor{mgood}{\textbf{\checkmark}}]
    \textbf{Differentiable} — gradient-based training
  \item[\textcolor{mgood}{\textbf{\checkmark}}]
    \textbf{$\mathcal{O}(N)$} — scale lên millions samples
  \item[\textcolor{mgood}{\textbf{\checkmark}}]
    \textbf{Provably correct} — convergence đến $\mathcal{N}(0,I)$
\end{itemize}
...
\begin{badbox}
\footnotesize \textbf{MMD} (Maximum Mean Discrepancy): provable nhưng $\mathcal{O}(N^2)$ — không scale.
\end{badbox}

\vspace{0.4em}
\begin{badbox}
\footnotesize \textbf{KL Divergence:} unstable gradient khi $p\approx 0$, không differentiable.
\end{badbox}
...
\begin{badbox}
\footnotesize\textbf{Theorem 3.} Với $K$ finite moments, minimizer \emph{không unique}: nhiều phân phối khác nhau đều có thể có cùng $K$ moment đầu như Gaussian.
\end{badbox}
...
\vspace{0.6em}
\begin{badbox}
\footnotesize\textbf{Không thể có cả hai:} tăng $K$ để nhận diện tốt hơn thì gradient bùng nổ → training mất ổn định.
\end{badbox}
...
\vspace{0.4em}
Họ test CDF đo khoảng cách giữa hai hàm phân bố tích luỹ. Tuy nhiên, việc tính toán thực nghiệm yêu cầu thao tác \textbf{sắp xếp (sort)} dữ liệu, gây ra hai hạn chế chí mạng cho học sâu:
...
\vspace{0.8em}
\footnotesize Multi-GPU phải synchronize toàn bộ data trước khi sort $\Rightarrow$ tạo ra nút thắt cổ chai nghiêm trọng.
\end{center}
...
\vspace{0.8em}
\footnotesize Cần differentiable sort relaxation $\Rightarrow$ sinh thêm hyperparameter và sai số xấp xỉ.
\end{center}
\end{columns}

\vspace{1.5em}
\takeaway{Hạn chế cốt lõi của CDF: Phép toán sort bẻ gãy tính toán song song và triệt tiêu luồng gradient}
\end{frame}
...
\vspace{0.6em}
\begin{primarybox}
\footnotesize \textbf{Theorem 4: Bounded Gradient}\\[2pt]
...
\vspace{0.4em}
\begin{goodbox}
\footnotesize \textbf{Theorem 4:} Gradient EP bị chặn bởi $\frac{4\sigma^2}{N}$ — không bao giờ explode.
\end{goodbox}
...
\begin{frame}[fragile]{SIGReg: Định Nghĩa Chính Thức + Implementation}
...
Max $\Rightarrow$ gradient sparse (chỉ 1 hướng được update). Average $\Rightarrow$ gradient dense, training ổn định hơn.
...
\textbf{Theorem 5 (Unified Error Bound):}\\[4pt]
...
\begin{axis}[
  ...
  xlabel={$M$ (số hướng, log-scale)}, ylabel={Expected error},
  xmode=log,
  xlabel style={font=\small}, ylabel style={font=\small},
  tick label style={font=\tiny},
...
\vspace{0.4em}
\begin{primarybox}
\footnotesize \textbf{2. SGD resampling:} Mỗi mini-batch resample $M$ hướng mới $\Rightarrow$ sau $T$ bước training, số hướng tích lũy $=M\times T$ $\Rightarrow$ coverage tăng miễn phí.
\end{primarybox}
...
\begin{primarybox}
\footnotesize \textbf{Defaults khuyên dùng:}\\[3pt]
$\lambda = 0.05$\\
...
\vspace{0.3em}
\begin{badbox}
\footnotesize \textbf{Theorem 3:} VICReg chỉ đối chiếu 2 moment đầu ($\hat\mu, \hat\sigma^2$) $\Rightarrow$ Dễ dính \textbf{shortcut} (nhiều phân phối khác nhau có cùng $\hat\mu, \hat\sigma^2$).\\[2pt]
$\Rightarrow$ Phải chắp vá bằng các heuristic phụ để tránh sụp đổ.
\end{badbox}

\vspace{0.6em}
\begin{goodbox}
\footnotesize \textbf{SIGReg:} Ràng buộc \textbf{toàn bộ} đặc trưng phân phối qua ECF $\Rightarrow$ Guarantee hội tụ tuyệt đối (Cramér-Wold).
\end{goodbox}
...
\textbf{Training loss dự báo downstream performance}
...
\column{0.48\textwidth}
\begin{primarybox}
\footnotesize Lần đầu trong SSL: loss train \textbf{tương quan 94\%} với accuracy — chọn model không cần linear probe.
\end{primarybox}
...
\begin{frame}{Ablation: LeJEPA Ổn Định Trên Mọi Hyperparameter}
...
\vspace{0.5em}
\begin{accentbox}
\footnotesize Không có collapse nào trong tất cả 50+ models, không tuning hyperparameter theo architecture.
\end{accentbox}
...
\vspace{0.8em}
\begin{primarybox}
\footnotesize \textbf{Lý do cốt lõi:} Mục tiêu $\mathcal{N}(0,I)$ ép mọi hướng trong không gian embedding đều phải chứa thông tin (maximum entropy) — triệt tiêu hoàn toàn hiện tượng "chiều chết" (dimensional collapse).
\end{primarybox}
\end{frame}
...
\begin{itemize}\small
  ...
  \item[\textcolor{mgood}{\textbf{\cmark}}] \textbf{Gọn nhẹ:} Chỉ 15M params, train trên \textbf{1 GPU}.
\end{itemize}
...
\begin{tikzpicture}[every node/.style={font=\small}, scale=0.88]
  \node[rbox, minimum width=2.8cm, minimum height=1.1cm, align=center]
    (old) at (0,0)
    {\textbf{Prior SSL}\\\small heuristic patches};

  \node[darkbox, minimum width=2.8cm, minimum height=1.1cm, align=center]
    (new) at (6.5,0)
    {\textbf{LeJEPA}\\\small first principles};
...
\begin{itemize}\small
  \item[\textcolor{mgood}{\textbf{\cmark}}]
    \textbf{Lý thuyết:} Iso Gaussian uniquely minimizes worst-case downstream risk
  \item[\textcolor{mgood}{\textbf{\cmark}}]
    \textbf{Method:} SIGReg — $\mathcal{O}(N)$, differentiable, bounded gradient
  \item[\textcolor{mgood}{\textbf{\cmark}}]
    \textbf{Practice:} SOTA trên 60+ architectures, beats DINOv2 in-domain
  \item[\textcolor{mgood}{\textbf{\cmark}}]
    \textbf{Simplicity:} 1 hyperparameter $\lambda$, ${\approx}50$ dòng code, không cần stop-gradient
\end{itemize}
\column{0.05\textwidth}
\end{columns}
