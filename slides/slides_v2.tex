...
\column{0.44\textwidth}
  \begin{center}
    ...
    {\large Provable \& Scalable SSL}\\[2pt]
    {\normalsize Without the Heuristics}\\[14pt]
    {\small Randall Balestriero \& Yann LeCun (2025)}\\[4pt]
    {\small\color{neutral} arXiv: 2511.08544}\\[12pt]
    {\footnotesize Presenter: [Tên bạn]}
  \end{center}
...
% ============================================================
\begin{frame}{SSL Học Từ Cấu Trúc Dữ Liệu — Không Cần Nhãn}
\begin{columns}[t]
\column{0.31\textwidth}
  ...
  \end{block}
\column{0.31\textwidth}
  \begin{block}{\centering Reconstruction}
    \centering\small Ảnh $\to$ Ảnh\\[4pt]
    \footnotesize
  ...
  \end{block}
\column{0.31\textwidth}
  \begin{exampleblock}{\centering Self-Supervised}
    \centering\small View1 $\leftrightarrow$ View2\\[4pt]
    \footnotesize
    \textcolor{good}{\cmark}\ Tự sinh tín hiệu\\
    \textcolor{good}{\cmark}\ Không cần nhãn\\
    \textcolor{good}{\cmark}\ Scale tốt
  \end{exampleblock}
\end{columns}
\takeaway{SSL: AI học từ cấu trúc dữ liệu — không từ nhãn con người}
\end{frame}
...
\column{0.47\textwidth}
  ...
  \begin{tikzpicture}[every node/.style={font=\tiny}]
    ...
    \node[circle, fill=bad, text=white, minimum size=0.7cm] (pt) at (2.3,-0.1) {$\mathbf{0}$};
    \draw[->, thick, color=bad] (x1) -- (pt);
    \draw[->, thick, color=bad] (x2) -- (pt);
    \draw[->, thick, color=bad] (x3) -- (pt);
    \draw[->, thick, color=bad] (x4) -- (pt);
    \node[below=3pt of pt, text=bad] {$f(x) =$ hằng số};
  \end{tikzpicture}
\column{0.47\textwidth}
  \begin{center}
  ...
  \begin{tikzpicture}[every node/.style={font=\tiny}]
    ...
    \node[embnode, minimum size=0.5cm] (p1) at (2.3, 1.0) {$z_1$};
    \node[embnode, minimum size=0.5cm] (p2) at (2.9, 0.1) {$z_2$};
    \node[embnode, minimum size=0.5cm] (p3) at (2.1,-0.6) {$z_3$};
    \node[embnode, minimum size=0.5cm] (p4) at (3.0,-1.1) {$z_4$};
    \draw[mainarrow] (y1) -- (p1);
    \draw[mainarrow] (y2) -- (p2);
    \draw[mainarrow] (y3) -- (p3);
    \draw[mainarrow] (y4) -- (p4);
  \end{tikzpicture}
\end{columns}
...
\column{0.57\textwidth}
  \centering
  \begin{tikzpicture}[every node/.style={font=\tiny}]
    % Hàng trên: view1 → encoder → z1 → so sánh
    \node[lightbluebox, minimum width=0.9cm, minimum height=0.55cm] (v1)  at (0.0, 0.7) {view 1};
    \node[primarybox,   minimum width=0.9cm, minimum height=0.55cm] (e1)  at (1.7, 0.7) {$f_\theta$};
    \node[accentbox,    minimum width=0.7cm, minimum height=0.55cm] (z1)  at (3.0, 0.7) {$z_1$};
    \node[goodbox,      minimum width=1.1cm, minimum height=0.55cm] (cmp) at (5.3, 0.7) {so sánh};
    % Hàng dưới: view2 → encoder → z2 → dự báo → z_hat
    \node[lightbluebox, minimum width=0.9cm, minimum height=0.55cm] (v2)  at (0.0,-0.7) {view 2};
    \node[primarybox,   minimum width=0.9cm, minimum height=0.55cm] (e2)  at (1.7,-0.7) {$f_\theta$};
    \node[accentbox,    minimum width=0.7cm, minimum height=0.55cm] (z2)  at (3.0,-0.7) {$z_2$};
    \node[neutralbox,   minimum width=1.1cm, minimum height=0.55cm] (pred) at (4.2,-0.7) {Dự báo};
    \node[accentbox,    minimum width=0.7cm, minimum height=0.55cm] (zh)  at (5.3,-0.7) {$\hat{z}_1$};
    % Mũi tên
    \draw[mainarrow] (v1) -- (e1);
    \draw[mainarrow] (e1) -- (z1);
    \draw[mainarrow] (z1) -- (cmp);
    \draw[mainarrow] (v2) -- (e2);
    \draw[mainarrow] (e2) -- (z2);
    \draw[mainarrow] (z2) -- (pred);
    \draw[mainarrow] (pred) -- (zh);
    \draw[->, thick, color=good] (zh) -- (cmp);
    % Dấu ngoặc chia sẻ trọng số
    \draw[decorate, decoration={brace, mirror, amplitude=4pt},
          color=neutral, thick]
      (1.25,-0.90) -- (1.25, 0.90)
      node[midway, left=6pt, font=\tiny, color=neutral] {chia sẻ};
  \end{tikzpicture}
\column{0.41\textwidth}
  ...
  \end{block}
  \vspace{4pt}
  \begin{alertblock}{\footnotesize JEPA vs LLM}
    \footnotesize
    \textcolor{bad}{\xmark}\ LLM: dự báo pixel\\
    \quad $\to$ lãng phí vào nhiễu\\[3pt]
...
% ============================================================
\begin{frame}{Heuristic Stack: Mỗi Phương Pháp Vá Một Lỗ — Không Có Nền Tảng}
\centering\footnotesize
\begin{tabular}{l >{\raggedright\arraybackslash}p{3.3cm} >{\raggedright\arraybackslash}p{3.9cm}}
\toprule
\textbf{Phương pháp} & \textbf{Cơ chế chống collapse} & \textbf{Hạn chế} \\
\midrule
SimCLR  & Negative samples (contrastive)
        & \cellcolor{bad!20}\textcolor{bad}{\xmark}\ $\mathcal{O}(N^2)$ bộ nhớ \\
BYOL    & Kiến trúc bất đối xứng + stop-grad
        & \cellcolor{bad!20}\textcolor{bad}{\xmark}\ Không có lý thuyết \\
DINO    & Teacher--student + EMA schedule
        & \cellcolor{bad!20}\textcolor{bad}{\xmark}\ $>$5 siêu tham số \\
VICReg  & Làm trắng phương sai (whitening)
        & \cellcolor{bad!20}\textcolor{bad}{\xmark}\ Under-specified \\
I-JEPA  & Stop-gradient + mask prediction
        & \cellcolor{bad!20}\textcolor{bad}{\xmark}\ Nhạy cảm, chỉ ViT \\
\bottomrule
\end{tabular}
...
% ============================================================
\begin{frame}[shrink=5]{Câu Hỏi Thay Đổi Mọi Thứ}
\begin{columns}[c]
\column{0.42\textwidth}
  \begin{block}{Cách tiếp cận cũ}
    \footnotesize
    \textcolor{bad}{\xmark}\ ``Làm sao \textbf{tránh} collapse?''\\[2pt]
    \textit{Vá từng lỗ hổng empirically}\\[4pt]
    \textcolor{bad}{\xmark}\ Không có lý thuyết nền\\
    \textcolor{bad}{\xmark}\ Mỗi fix tạo ra vấn đề mới\\
    \textcolor{bad}{\xmark}\ Không biết khi nào dừng
  \end{block}
  \vspace{2pt}
  \begin{exampleblock}{Câu hỏi LeJEPA}
    \footnotesize
    \textcolor{good}{\cmark}\ ``Distribution \textbf{nào} là tối ưu?''\\[2pt]
    \textit{Giải từ gốc rễ}\\[4pt]
    \textcolor{good}{\cmark}\ Định lý rõ ràng\\
    \textcolor{good}{\cmark}\ Collapse tự bị loại trừ\\
    \textcolor{good}{\cmark}\ Biết chính xác cần đạt gì
  \end{exampleblock}
\column{0.56\textwidth}
  \centering
  \begin{tikzpicture}[every node/.style={font=\small}, scale=0.84, transform shape]
    % Node trái: vấn đề cũ
    \node[badbox, minimum width=2.8cm, minimum height=1.0cm,
          text width=2.6cm, align=center]
      (old) at (0, 1.3) {\footnotesize Prevent\\Collapse};
    % Node giữa: câu hỏi mới
    \node[primarybox, minimum width=3.0cm, minimum height=1.0cm,
          text width=2.8cm, align=center]
      (pivot) at (0,-0.2) {\footnotesize\bfseries Distribution\\Tối Ưu?};
    % Node dưới: kết quả
    \node[goodbox, minimum width=2.8cm, minimum height=1.0cm,
          text width=2.6cm, align=center]
      (result) at (0,-1.7) {\footnotesize $\mathcal{N}(0,I)$\\Optimal};
    % Mũi tên
    \draw[->, thick, color=bad!70, dashed]
      (old) -- node[right=4pt, font=\tiny, color=neutral]
      {đổi góc nhìn} (pivot);
    \draw[->, thick, color=good]
      (pivot) -- node[right=4pt, font=\tiny, color=neutral]
      {Theorem 3.3} (result);
    % 3 axioms bên phải
    \node[lightbluebox, font=\tiny, text width=2.2cm, align=left,
          minimum height=0.65cm]
      (ax1) at (4.2, 1.2) {1.\ Giải bài toán\\dự báo (cũ)};
    \node[primarybox, font=\tiny, text width=2.2cm, align=left,
          minimum height=0.65cm]
      (ax2) at (4.2, 0.0) {2.\ Enforce iso\\Gaussian \textbf{(MỚI)}};
    \node[goodbox, font=\tiny, text width=2.2cm, align=left,
          minimum height=0.65cm]
      (ax3) at (4.2,-1.2) {3.\ Không thêm\\gì khác};
    \draw[subarrow] (result.east) to[out=0,in=180] (ax1.west);
    \draw[subarrow] (result.east) to[out=0,in=180] (ax2.west);
    \draw[subarrow] (result.east) to[out=0,in=180] (ax3.west);
  \end{tikzpicture}
\end{columns}
...
\column{0.48\textwidth}
  \begin{alertblock}{\textcolor{bad}{\xmark}\ Anisotropic $\mathbf{Z}_{\text{aniso}}$}
    \footnotesize
    Covariance eigenvalues:\\
    $\lambda_1 \gg \lambda_2 \gg \cdots \gg \lambda_K$\\[4pt]
    \textcolor{bad}{$\Rightarrow$ Một số chiều bị ``squished''}\\
    \textcolor{bad}{$\Rightarrow$ Estimator lean vào chiều lớn}\\[4pt]
    \textbf{Hệ quả:}\\
    $\text{Bias}(\hat\beta_{\text{aniso}})$ cao\\
    $\text{tr}(\text{Var}(\hat\beta_{\text{aniso}}))$ cao
  \end{alertblock}
\column{0.48\textwidth}
  \begin{exampleblock}{\textcolor{good}{\cmark}\ Isotropic $\mathbf{Z}_{\text{iso}}$}
    \footnotesize
    Covariance eigenvalues:\\
    $\lambda_1 = \lambda_2 = \cdots = \lambda_K = \tfrac{1}{K}\sum_k\lambda_k$\\[4pt]
    \textcolor{good}{$\Rightarrow$ Mọi chiều đều quan trọng như nhau}\\
    \textcolor{good}{$\Rightarrow$ Estimator cân bằng}\\[4pt]
    \textbf{Hệ quả:}\\
    $\text{Bias}(\hat\beta_{\text{iso}})$ thấp hơn\\
    $\text{tr}(\text{Var}(\hat\beta_{\text{iso}}))$ thấp hơn
  \end{exampleblock}
\end{columns}
...
\column{0.48\textwidth}
  ...
  \begin{axis}[
    ...
    xmin=50, xmax=500,
    ymin=0.3, ymax=1.05,
    legend style={font=\tiny, at={(0.98,0.08)},
                  anchor=south east, draw=none,
                  fill=lightgray, inner sep=2pt},
    legend cell align={left},
    grid=major, grid style={gray!20},
  ]
    \addplot[color=bad, thick, mark=none, smooth]
      coordinates {(50,0.38)(100,0.52)(200,0.67)(300,0.76)(400,0.83)(500,0.87)};
    \addlegendentry{Aniso \xmark}
    \addplot[color=good, thick, mark=none, smooth]
      coordinates {(50,0.61)(100,0.74)(200,0.85)(300,0.90)(400,0.94)(500,0.96)};
    \addlegendentry{Iso \cmark}
  \end{axis}
  \end{tikzpicture}
  \begin{alertblock}{\tiny Lemma 3.1 — Bias}
    \tiny Với $\lambda>0$, $\exists\,\mathbf{y}$:\;$\text{Bias}(\hat\beta_{\text{aniso}}) > \text{Bias}(\hat\beta_{\text{iso}})$
  \end{alertblock}

% --- Panel phải: Variance ---
\column{0.48\textwidth}
  \begin{center}\footnotesize\textbf{Variance của decision boundary}\end{center}
  \begin{tikzpicture}
  \begin{axis}[
    width=5.2cm, height=2.7cm,
    xlabel={Dim.~1},
    ...
    \addplot[color=bad, thin, domain=-2.8:2.8, samples=2]
      {0.60*x + 0.15};
    \addplot[color=bad, thin, domain=-2.8:2.8, samples=2]
      {0.60*x - 0.30};
    \addplot[color=bad, thin, domain=-2.8:2.8, samples=2]
      {0.60*x + 0.55};
    \addlegendentry{Aniso \xmark}
    \addplot[color=good, thick, domain=-2.8:2.8, samples=2]
      {-0.55*x + 0.05};
    \addlegendentry{Iso \cmark}
  \end{axis}
  \end{tikzpicture}
  \begin{alertblock}{\tiny Lemma 3.2 — Variance}
    \tiny $\text{tr}(\text{Var}(\hat\beta_{\text{aniso}})) > \text{tr}(\text{Var}(\hat\beta_{\text{iso}}))$
  \end{alertblock}
\end{columns}
