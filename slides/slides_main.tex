...
\vspace{0.65cm}
\begin{center}
\begin{tikzpicture}[x=2.60cm, y=1cm]
  \tikzstyle{stepbox} = [draw=mprimary!40, fill=white, rounded corners=6pt, text width=1.90cm, align=center, inner sep=4pt, minimum height=1.55cm, line width=1.2pt]
  \tikzstyle{stepnum} = [circle, fill=mprimary, text=white, font=\large\bfseries, inner sep=0pt, minimum size=0.8cm]
  
  %% Connecting Line
  \draw[line width=2pt, mprimary!20] (0, 1.5) -- (4, 1.5);
  ...
  %% Step 5
  \node[stepbox, draw=mprimary!85!mgood] (b5) at (4, -0.2) {\textbf{\color{mprimary!85!mgood}EXTENSION}\\[3pt]\scriptsize Nghiên cứu mở rộng CLIMB};
...
% ─── [MERGED] Bổ đề 1 + Bổ đề 2 + Định lý 1 + Kết luận → 1 slide tóm gọn ──────
% 4 frame gốc (Bổ đề 1, Bổ đề 2, Định lý 1, Kết Luận Lý Thuyết) bị comment lại
% vì lặp nội dung với slide "Phân phối Nào Là Tối Ưu Cho Foundation Model?"
% (đã có cả 2 bổ đề + định lý trong các box) và với nhau (mỗi slide chỉ thêm
% 1 chart minh họa nhưng cùng kết luận). Gộp thành 1 slide nhẹ toán, giữ
% trực giác bias/variance + phát biểu định lý + design principle.
\begin{frame}{Tóm Tắt: Vì Sao Isotropic Gaussian Là Tối Ưu}

\begin{columns}[T]
\column{0.33\textwidth}
\begin{badbox}
\footnotesize\textbf{Bổ đề 1 — Bias}\\[3pt]
Hướng có trị riêng nhỏ bị ridge co mạnh hơn $\Rightarrow$ anisotropic luôn có task khiến bias cao hơn isotropic.
\end{badbox}

\column{0.33\textwidth}
\begin{badbox}
\footnotesize\textbf{Bổ đề 2 — Variance}\\[3pt]
Trị riêng không đều $\Rightarrow$ decision boundary dao động mạnh hơn giữa các lần resample $\Rightarrow$ variance cao hơn.
\end{badbox}

\column{0.33\textwidth}
\begin{goodbox}
\footnotesize\textbf{Định lý 1 — Duy nhất}\\[3pt]
Với cùng năng lượng $\mathrm{Tr}(\mathrm{Cov})=\kappa$, $\mathcal{N}(0,\kappa I/K)$ là điểm cực tiểu \textbf{duy nhất} của downstream risk — đúng cho linear probe, kNN, kernel.
\end{goodbox}
\end{columns}

\vspace{0.8em}
\begin{center}
{\Large\bfseries\color{mprimary} $f_\theta(\mathbf{x}) \sim \mathcal{N}(\mathbf{0}, I)$}\\[4pt]
{\small\color{mgray} Design principle duy nhất cần đạt — không cần heuristic}
\end{center}
...
% BỊ LẶP: minh họa aniso-ellipsoid vs iso-sphere ở đây gần như trùng với slide
% "Phân phối Nào Là Tối Ưu Cho Foundation Model?" (3D ellipsoid/sphere TikZ ở
% trên) — chỉ khác là thêm khung "kNN neighborhood". Comment lại, có thể giữ
% làm slide phụ lục (backup) nếu cần đào sâu trực giác kNN khi Q&A.
\iffalse
...
\midrule
\rowcolor{mgood!15}
\textbf{SIGReg (Ours)} & \textcolor{mgood}{\textbf{\cmark}} & \textcolor{mgood}{\textbf{\cmark}} & \textcolor{mgood}{\textbf{\cmark}} \\
\bottomrule
\end{tabular}
\end{center}
...
\small
\textbf{Empirical Characteristic Function (ECF):}
\[ \hat{\phi}_X(t) = \frac{1}{N}\sum_{n=1}^N e^{it X_n} \]

\vspace{0.3em}
\textbf{Epps-Pulley (EP):} Tối thiểu hoá sai số:
...
\column{0.32\textwidth}
\begin{goodbox}
\centering \textbf{Differentiable}\\[4pt]
\footnotesize Không cần sort $\Rightarrow$ $\nabla$ mượt mà.
\end{goodbox}

\column{0.32\textwidth}
\begin{goodbox}
\centering \textbf{Scalable $\mathcal{O}(N)$}\\[4pt]
\footnotesize Phép tổng $\sum$ dễ \texttt{all\_reduce}.
\end{goodbox}

\column{0.33\textwidth}
\begin{accentbox}
\centering \textbf{Provably Stable}\\[4pt]
\footnotesize $|e^{itx}|=1 \Rightarrow$ Gradient giới hạn.
\end{accentbox}
\end{columns}
\end{frame}
...
\textbf{Chiều dễ thấy:}
\[
  \mathbf{Z}\sim\mathcal{N}(\mathbf{0},I_K),\quad
  \|\mathbf{u}\|_2=1
\]
\[
  \mathbf{u}^{\top}\mathbf{Z}
  \sim
  \mathcal{N}\!\left(0,\mathbf{u}^{\top}I_K\mathbf{u}\right)
  =
  \mathcal{N}(0,1).
\]
...
\begin{goodbox}
\footnotesize
Nghĩa là nhìn từ \textbf{bất kỳ hướng nào}, embedding chuẩn đều cho cùng một đường cong chuông chuẩn 1D.
\end{goodbox}

\vspace{0.45em}
\begin{primarybox}
\footnotesize
\textbf{Chiều ngược lại -- Cramér-Wold:}\\[2pt]
...
\vspace{-0.65em}
\begin{center}
\includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{sigreg_gaussian_projection_line.png}
\end{center}

\vspace{-0.75em}
{\tiny\color{mgray}
\textbf{Ghi chú:} Hình tạo bằng matplotlib từ dữ liệu mô phỏng Gaussian để trực quan hóa các hình chiếu 1D của Cramér-Wold/SIGReg; không phải kết quả thực nghiệm.
}
...
\centering\footnotesize\bfseries\color{mprimary}
...
\textbf{Từ trực giác sang công thức:}
\[
  X \stackrel{d}{=} Y
  \iff
...
\begin{accentbox}
\footnotesize
Với mỗi minibatch embedding $\{z_n\}_{n=1}^{N}$, lấy $M$ hướng ngẫu nhiên và kiểm tra từng dãy vô hướng
$\{\mathbf{u}_m^\top z_n\}_{n=1}^{N}$ so với $\mathcal{N}(0,1)$.
\end{accentbox}
...
\[
\begin{aligned}
EP &= N\!\int |\hat\phi_X(t) - \phi_0(t)|^2 w(t)\,dt,\\[-1pt]
\hat\phi_X(t)&=\frac{1}{N}\sum_{j=1}^{N} e^{itX_j}.
\end{aligned}
\]
}
...
\begin{primarybox}
\footnotesize\textbf{SIGReg}
\[
  \mathrm{SIGReg}_T
  =
  \frac{1}{M}\sum_{m=1}^{M}
  T\!\left(\{\mathbf{u}_m^\top z_n\}_{n=1}^{N}\right)
\]
\end{primarybox}

\vspace{0.55em}
\begin{goodbox}
\footnotesize \textbf{Ý nghĩa tối ưu:}\\[2pt]
Minimize SIGReg $\Rightarrow$ mọi hình chiếu 1D tiến gần $\mathcal{N}(0,1)$ $\Rightarrow$ toàn bộ embedding tiến về $\mathcal{N}(0,I)$.
\end{goodbox}

\vspace{0.55em}
\begin{goodbox}
\footnotesize \textbf{Định lý 4:} Gradient EP bị chặn bởi $\frac{4\sigma^2}{N}$.
\end{goodbox}

\vspace{0.55em}
\begin{accentbox}
\footnotesize $M=16$ hướng thường đủ nhờ tính trơn của DNN + SGD resampling tích lũy.\\[2pt]
...
\vspace{1em}
\emph{Chú thích: dùng average thay vì max $\rightarrow$ gradient dense.}
...
\vspace{-0.8em}
\begin{center}
\hspace*{-0.8em}\resizebox{1.05\linewidth}{!}{
\begin{tikzpicture}[
...
\scriptsize
\textbf{Epps-Pulley} — top1 (\%), đủ 27 cấu hình:\\[0.2em]
\begin{tabular}{@{}llccc@{}}
\toprule
...
\vspace{0.4em}
\textbf{Số view} ($V$): \; 4:\,\textbf{72.8} \; 6:\,65.7 \; 8:\,60.6 \; 10:\,57.1
...
\takeaway{Chỉ \texttt{num\_slices} là đòn bẩy thật — nhiều lát cắt cho ước lượng SIGReg mịn hơn; \texttt{n\_points}/$t_{\max}$ gần trung tính, cho thấy SIGReg ổn định với lưới tích phân (đúng như paper)}
...
{\centering\includegraphics[width=0.80\linewidth]{E_arch_bars.pdf}\par}
...
\takeaway{Collapse có ranh giới rõ theo thiết kế — chỉ xảy ra khi áp SIGReg sai chỗ (embedding) hoặc projector không đủ phi tuyến; đặt đúng thì SIGReg tự chống collapse, khỏi cần predictor}
...
\takeaway{Trái kỳ vọng: tắt mask \& drop-path lại tốt nhất (mô hình chưa ở chế độ overfit)}

\end{frame}


% % ─── [BATCH 4] Emergent semantics ────────────────────────────────────────────
% Disabled for the current main slide flow.
\iffalse
...
\begin{primarybox}
\footnotesize
\textbf{Bối cảnh:} Sau pretrain \kw{LeJEPA}, backbone được freeze trong bước \kw{frozen-linear eval}; chỉ train probe tuyến tính và baseline trên Imagenette vẫn rất mạnh.
\end{primarybox}

\vspace{0.6em}
\footnotesize
\textbf{\color{mprimary}Hai giả thuyết còn lại:}
\begin{itemize}\itemsep3pt
  \item \textbf{Objective / representation head:} thêm tín hiệu hình học hoặc đổi projector có thể cải thiện SIGReg.
  \item \textbf{Training geometry / optimizer / inductive bias:} mục tiêu có thể ổn, nhưng dynamics hoặc stem của ViT còn là nút nghẽn.
\end{itemize}

\column{0.46\textwidth}
\begin{accentbox}
\footnotesize
\textbf{Giao thức đánh giá:}\\[0.25em]
\scriptsize
\kw{Backbone}: frozen encoder.\\[0.15em]
\kw{Feature}: concat CLS từ 2 block cuối + LayerNorm.\\[0.15em]
\kw{Probe}: linear classifier với AdamW.\\[0.15em]
\kw{Metric}: top1 và $\Delta$ so với \textbf{baseline cùng setup}.
\end{accentbox}

\vspace{0.55em}
\begin{goodbox}
\centering\footnotesize
\textbf{Mốc tham chiếu:}\\[0.2em]
\kw{Objective/head}: \textbf{89.54\%}\\
\kw{Optimizer/training}: \textbf{89.49\%}
\end{goodbox}

\end{columns}

\takeaway{Câu hỏi không phải ``đã thử bao nhiêu'', mà là headroom nằm ở mục tiêu, dynamics huấn luyện, hay inductive bias của backbone}
...
\textbf{\color{mprimary}Đọc kết quả:}\\[0.25em]
\scriptsize
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}p{1.55cm}p{3.25cm}@{}}
\toprule
\textbf{Can thiệp} & \textbf{Diễn giải} \\
\midrule
...
\begin{insightbox}
\scriptsize
\textbf{Insight:} Các can thiệp trực tiếp vào \kw{geometry} không tạo \kw{headroom rõ}; càng mạnh tay càng dễ phá cân bằng của baseline.
\end{insightbox}
...
\begin{goodbox}
\scriptsize
\textbf{Điểm cân bằng hiện tại}\\[0.2em]
Baseline nối \kw{invariance} với \kw{SIGReg} qua \kw{projector MLP} đủ capacity. Đây không chỉ là chi tiết kỹ thuật; nó là interface phân phối.
\end{goodbox}

\vspace{0.45em}
\begin{accentbox}
\scriptsize
\textbf{Vì sao kết quả âm vẫn hữu ích}\\[0.2em]
\begin{itemize}\itemsep2pt
  \item \textbf{Uniformity} gần baseline: thêm spread nhẹ chưa tạo lợi thế.
  \item \textbf{Coding-rate collapse}: auxiliary geometry có thể xung đột với SIGReg.
  \item \textbf{DynTanh thất bại}: projector normalization không phải phần có thể bỏ tùy ý.
\end{itemize}
\end{accentbox}
...
\begin{insightbox}
\scriptsize
\textbf{Kết luận hướng 1:} Muốn cải thiện \kw{objective}, cần thiết kế cùng \kw{projector}; không gắn thêm loss như một thành phần độc lập.
\end{insightbox}

\end{columns}

\takeaway{Bài học chính: LeJEPA không chỉ là loss; SIGReg, invariance, và projector MLP tạo thành một cân bằng khá mong manh}
...
\centering
\includegraphics[width=\linewidth]{fig_climb_optbars.pdf}

\vspace{0.15em}
{\scriptsize\color{mgray} Frozen top1 delta so với baseline 89.49\%; conv-stem được tách khỏi chart này.}
...
\begin{primarybox}
\scriptsize
\textbf{Cụm gần baseline}\\[0.15em]
\textbf{SAM} (+0.23pp), \textbf{stoch-depth} (+0.01pp), \textbf{PCGrad} (-0.05pp): sát baseline, chưa là thắng lợi rõ.
\end{primarybox}

\vspace{0.22em}
\begin{accentbox}
\scriptsize
\textbf{Nhóm cơ chế}\\[0.15em]
\kw{Flat minima / schedule}: SAM, SWA, stoch-depth.\\
\kw{Optimizer replacement}: Muon, Schedule-Free, LLRD.\\
\kw{Gradient geometry}: PCGrad.\\
\kw{Attention / depth}: QK-Norm, deep supervision.
\end{accentbox}

\vspace{0.22em}
\begin{badbox}
\scriptsize
\textbf{Muon} (-3.75pp) và \textbf{LLRD} (-3.33pp) làm xấu frozen features rõ rệt.
\end{badbox}
...
\column{0.52\textwidth}
\centering
\includegraphics[width=\linewidth]{fig_climb_convstem.pdf}

\vspace{0.15em}
{\scriptsize\color{mgray} Thay patchify stride-16 bằng stem 4 conv; ViT body và CLS readout giữ tương thích.}
...
\begin{goodbox}
\scriptsize
\textbf{Kết quả rõ nhất}\\[0.15em]
\kw{Patchify baseline}: \textbf{89.49\%}\\
\kw{Conv-stem}: \textbf{92.08\%}\\
$\Delta$: \textbf{+2.59pp}
\end{goodbox}
...
\begin{primarybox}
\scriptsize
\textbf{Nguồn ý tưởng:} Xiao et al., \emph{Early Convolutions Help Transformers See Better}, NeurIPS 2021 \citep{xiao2021early}.\\[0.2em]
\kw{Early convs} thêm locality / translation bias trước transformer blocks.
\end{primarybox}

\vspace{0.45em}
\begin{badbox}
\scriptsize
\textbf{Giới hạn diễn giải:} Đây là cải thiện về \emph{backbone inductive bias}, chưa phải bằng chứng rằng objective LeJEPA tốt hơn.
\end{badbox}

\end{columns}

\takeaway{Conv-stem cho thấy headroom đáng kể đến từ kiến trúc đầu vào; vì vậy kết quả này nên được báo cáo như architecture co-design, không gộp với objective hay optimizer}
