...
\vspace{0.65cm}
\begin{center}
\begin{tikzpicture}[x=3.15cm, y=1cm]
  \tikzstyle{stepbox} = [draw=mprimary!40, fill=white, rounded corners=6pt, text width=2.35cm, align=center, inner sep=4pt, minimum height=1.55cm, line width=1.2pt]
  ...
  %% Step 2
  \node[stepbox, draw=mgood] (b2) at (1, -0.2) {\textbf{\color{mgood}HOW}\\[3pt]\scriptsize Giải quyết từ gốc rễ};
  \node[stepnum, fill=mgood] (n2) at (1, 1.5) {2};
  \draw[thick, mgood] (n2) -- (b2);

  %% Step 3
  \node[stepbox, draw=maccent] (b3) at (2, -0.2) {\textbf{\color{maccent}RESULTS}\\[3pt]\scriptsize Đánh giá thực nghiệm};
  \node[stepnum, fill=maccent] (n3) at (2, 1.5) {3};
  \draw[thick, maccent] (n3) -- (b3);

  %% Step 4
  \node[stepbox, draw=mprimary!85!mgood] (b4) at (3, -0.2) {\textbf{\color{mprimary!85!mgood}EXTENSION}\\[3pt]\scriptsize Nghiên cứu mở rộng CLIMB};
  \node[stepnum, fill=mprimary!85!mgood] (n4) at (3, 1.5) {4};
  \draw[thick, mprimary!85!mgood] (n4) -- (b4);
...
% ─── Slide 1+2 [MERGED]: Mục tiêu học biểu diễn + cơ chế JEPA ─────────────────
\begin{frame}{Mục Tiêu Học Biểu Diễn \& Cơ Chế JEPA}

\begin{columns}[T]
\column{0.40\textwidth}

Học $f_\theta : \mathbb{R}^D \to \mathbb{R}^K$ cho embedding \textbf{hữu ích cho mọi task} — không cần nhãn.
...
\textbf{Cơ chế JEPA:} dự đoán trạng thái latent của view này từ view kia — không khôi phục pixel.

\vspace{0.3em}
\begin{center}
\begin{tikzpicture}[every node/.style={font=\tiny}, scale=0.80, transform shape]
  % Ảnh gốc
...
\vspace{0.3em}
Nhưng có nghiệm tầm thường: mọi $z$ về cùng một điểm $\to$ \textbf{collapse}.
...
% ─── SECTION DIVIDER ──────────────────────────────────────────────────────────
% \section{WHY — Tại sao JEPA thất bại?}  % [HIDDEN] các frame WHY đã ẩn


\iffalse % ─── [HIDDEN] frames của phần WHY ───────────────────────────────────
...
\iffalse % ─── [HIDDEN] frame "Phân phối nào tối ưu" ────────────────────────
...
\end{frame}
\fi % ─── [HIDDEN] hết frame "Phân phối nào tối ưu" ──────────────────────────
...
\iffalse % ─── [HIDDEN] frame "λ: Nút Điều Chỉnh Duy Nhất" ─────────────────────
...
\end{columns}
\end{frame}
\fi % ─── [HIDDEN] hết frame "λ: Nút Điều Chỉnh Duy Nhất" ────────────────────


\iffalse % ─── [HIDDEN] frame "LeJEPA Generalizes VICReg" ──────────────────────
...
\end{columns}
\end{frame}
\fi % ─── [HIDDEN] hết frame "LeJEPA Generalizes VICReg" ─────────────────────
...
{\footnotesize\color{mgray}Giả thuyết 1}\hfill{\footnotesize\color{mgray}baseline $\approx$ \textbf{\color{mprimary}89.5\%}}\\[5pt]
...
\begin{mdframed}[roundcorner=8pt, linewidth=1.2pt, linecolor=mgood!60,
    ...
    innerleftmargin=11pt, innerrightmargin=11pt]
{\footnotesize\color{mgray}Giả thuyết 2}\hfill{\footnotesize\color{mgray}baseline $\approx$ \textbf{\color{mgood!60!black}89.5\%}}\\[5pt]
