"""LeJEPA ViT-S/T on ImageNet-10 — Variant 2: Hyvärinen score matching.

Idea: Batch-7 Idea 2 (TOY Phase B).
Replaces SlicedEppsPulley with full-d Hyvärinen ISM + Hutchinson trace.

Usage examples
--------------
python lejepa-hyvarinen.py --backbone vit_small_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf

python lejepa-hyvarinen.py --backbone vit_tiny_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf
"""
...
from stable_pretraining.methods.lejepa_variants import HyvarienSIGReg
...
def get_args():
    p = argparse.ArgumentParser(description="LeJEPA + Hyvärinen score matching on ImageNet-10")
...
def main():
    ...
    run_name = args.run_name or f"lejepa-hyvarinen-{btag}-inet10"
    ckpt_dir = args.checkpoint_dir or str(Path(__file__).parent / "checkpoints" / f"lejepa-hyvarinen-{btag}")
    ...
    model.sigreg = HyvarienSIGReg(dim=args.proj_dim)
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=ckpt_dir, filename=f"lejepa-hyvarinen-{btag}-{{epoch:03d}}",
