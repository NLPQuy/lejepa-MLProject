"""LeJEPA ViT-S on ImageNet-10 — Variant 2: Hyvärinen score matching.

Idea: Batch-7 Idea 2 (TOY, Phase B).
Change from baseline: replace SlicedEppsPulley with full-d Hyvärinen ISM
+ Hutchinson trace estimator. Zero new hyperparameters (~10 LoC core).
Expected gain: +0.5 pp (lower-variance full-d estimator vs per-slice ECF test).
...
def main():
    ...
    model.sigreg = HyvarienSIGReg(dim=PROJ_DIM)  # <--
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(Path(__file__).parent / "checkpoints" / "lejepa-hyvarinen"),  # <--
                filename="lejepa-hyvarinen-{epoch:03d}",
                save_top_k=-1,
                every_n_epochs=300,
                save_last=True,
            ),
