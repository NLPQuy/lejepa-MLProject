"""LeJEPA ViT-S on ImageNet-10 — Variant 3: Adversarial max-sliced SIGReg.

Idea: Batch-7 Idea 3 (TOY → FULL SEND on Phase A pass).
Change from baseline: replace M=1024 random slices with a learned adversarial
slicing head g_φ that finds the worst-case projection direction (minimax).
Expected gain: +0.7 pp (worst-case coverage; also 1000× cheaper SIGReg/step).

Single-optimizer detach trick:
  enc_loss = ep(z @ u.detach())    encoder descends
  adv_loss = -ep(z.detach() @ u)  adversary ascends (minimises negative)
  total    = enc_loss + adv_loss   ← ~0 numerically; gradients are non-zero
...
from stable_pretraining.methods.lejepa_variants import AdversarialSIGReg  # <--

PROJ_DIM = 512  # must match LeJEPA projector_dim default
...
def main():
    ...
    model.sigreg = AdversarialSIGReg(dim=PROJ_DIM)  # <--
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(Path(__file__).parent / "checkpoints" / "lejepa-adversarial"),  # <--
                filename="lejepa-adversarial-{epoch:03d}",
                save_top_k=-1,
                every_n_epochs=300,
                save_last=True,
            ),
        ...
        logger=pl.pytorch.loggers.WandbLogger(
            ...
            name="lejepa-adversarial-inet10",  # <--
            log_model=False,
        ),
        precision="16-mixed",
        devices=num_gpus,
