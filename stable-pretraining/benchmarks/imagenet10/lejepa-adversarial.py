"""LeJEPA ViT-S/T on ImageNet-10 — Variant 3: Adversarial max-sliced SIGReg.

Idea: Batch-7 Idea 3 (TOY → FULL SEND on Phase A).
Minimax slicing: adversary finds worst-case direction, encoder defends.
Single-optimizer via detach trick (no manual optimization needed).

Usage examples
--------------
python lejepa-adversarial.py --backbone vit_small_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf

python lejepa-adversarial.py --backbone vit_tiny_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf
"""
...
def get_args():
    p = argparse.ArgumentParser(description="LeJEPA + adversarial max-sliced SIGReg on ImageNet-10")
...
def main():
    ...
    run_name = args.run_name or f"lejepa-adversarial-{btag}-inet10"
    ckpt_dir = args.checkpoint_dir or str(Path(__file__).parent / "checkpoints" / f"lejepa-adversarial-{btag}")
    ...
    model.sigreg = AdversarialSIGReg(dim=args.proj_dim)
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=ckpt_dir, filename=f"lejepa-adversarial-{btag}-{{epoch:03d}}",
