"""LeJEPA ViT-S/T on ImageNet-10 — Baseline.

Usage examples
--------------
# ViT-S, 100 epochs, local dataset
python lejepa-vit-small.py --backbone vit_small_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf

# ViT-T, 100 epochs, HuggingFace download
python lejepa-vit-small.py --backbone vit_tiny_patch16_224 --max_epochs 100

# Override wandb run name
python lejepa-vit-small.py --run_name my-baseline-vits
"""
...
# ---------------------------------------------------------------------------
# Shared helpers (identical across all variant scripts)
# ---------------------------------------------------------------------------

def _backbone_tag(name: str) -> str:
    """vit_small_patch16_224 → vits, vit_tiny_patch16_224 → vitt, etc."""
    parts = name.split("_")
...
def get_args():
    p = argparse.ArgumentParser(description="LeJEPA baseline on ImageNet-10")
    ...
    p.add_argument("--data_local_path",  default=None,
                   help="Path to local imagenette saved via save_to_disk(). "
                        "Expected: <path>/train/ and <path>/validation/")
...
def main():
    ...
    run_name      = args.run_name or f"lejepa-baseline-{btag}-inet10"
    ckpt_dir      = args.checkpoint_dir or str(Path(__file__).parent / "checkpoints" / f"lejepa-baseline-{btag}")
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=ckpt_dir, filename=f"lejepa-baseline-{btag}-{{epoch:03d}}",
