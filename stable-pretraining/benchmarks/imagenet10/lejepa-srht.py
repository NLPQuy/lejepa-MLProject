"""LeJEPA ViT-S/T on ImageNet-10 — Variant 1: SRHT structured slices.

Idea: Batch-3 Idea 2 (FULL SEND).
Replace Gaussian random projections with SRHT (Walsh-Hadamard + Rademacher).

Usage examples
--------------
python lejepa-srht.py --backbone vit_small_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf

python lejepa-srht.py --backbone vit_tiny_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf
"""
...
from stable_pretraining.methods.lejepa_variants import SRHTSlicedEppsPulley
...
def get_args():
    p = argparse.ArgumentParser(description="LeJEPA + SRHT slices on ImageNet-10")
...
def main():
    ...
    run_name = args.run_name or f"lejepa-srht-{btag}-inet10"
    ckpt_dir = args.checkpoint_dir or str(Path(__file__).parent / "checkpoints" / f"lejepa-srht-{btag}")
    ...
    model.sigreg = SRHTSlicedEppsPulley(d=args.proj_dim, num_slices=args.n_slices)
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=ckpt_dir, filename=f"lejepa-srht-{btag}-{{epoch:03d}}",
