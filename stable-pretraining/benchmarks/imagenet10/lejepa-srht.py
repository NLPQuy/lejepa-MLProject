"""LeJEPA ViT-S on ImageNet-10 — Variant 1: SRHT structured slices.

Idea: Batch-3 Idea 2 (FULL SEND).
Change from baseline: replace Gaussian random projections in SlicedEppsPulley
with SRHT (Subsampled Randomized Hadamard Transform) projections.
Expected gain: +0.6 pp (lower SIGReg variance → stabler training).
...
def main():
    ...
    model.sigreg = SRHTSlicedEppsPulley(d=PROJ_DIM, num_slices=1024)  # <--
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(Path(__file__).parent / "checkpoints" / "lejepa-srht"),  # <--
                filename="lejepa-srht-{epoch:03d}",
                save_top_k=-1,
                every_n_epochs=300,
                save_last=True,
            ),
