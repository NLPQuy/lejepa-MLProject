"""LeJEPA ViT-S on ImageNet-10 — Variant 4: FM-SIGReg v2.

Idea: Batch-7 Idea 7 (FULL SEND after targeted research upgrades).
Change from baseline: replace SlicedEppsPulley with flow-matching transport
regularizer toward N(0, I). Velocity field v_ψ(z_t, t) trained jointly with
the encoder. v2 upgrades: ExFM closed-form target (A), KL bound (B),
Hungarian OT coupling within batch (C).
Expected gain: +0.6 pp (transport-based mechanism distinct from all test-based).
...
def main():
    ...
    model.sigreg = FMSIGReg(dim=PROJ_DIM)  # <--
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(Path(__file__).parent / "checkpoints" / "lejepa-fm-sigreg"),  # <--
                filename="lejepa-fm-sigreg-{epoch:03d}",
                save_top_k=-1,
                every_n_epochs=300,
                save_last=True,
            ),
