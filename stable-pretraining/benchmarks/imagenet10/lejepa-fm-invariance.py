"""LeJEPA ViT-S on ImageNet-10 — Variant 5: FM-Invariance.

Idea: Batch-7 Idea 1 (TOY).
Change from baseline: replace per-pair MSE invariance with flow-matching
distributional alignment. A velocity field v_ψ(z_t, t) is trained to transport
the global-view center distribution to each predicted view distribution.
SIGReg (EP test) is unchanged.
Expected gain: +0.8 pp (softer alignment; view-specific information preserved).

Implementation: LeJEPAFMInv subclass overrides _compute_loss via Python MRO.

Only lines that differ from lejepa-vit-small.py are marked with  # <--
"""
...
from stable_pretraining.methods.lejepa_variants import LeJEPAFMInv  # <--

PROJ_DIM = 512  # must match projector_dim
...
def main():
    ...
    # <-- LeJEPAFMInv instead of LeJEPA; projector_dim captured for fm_inv dim
    model = LeJEPAFMInv(
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(Path(__file__).parent / "checkpoints" / "lejepa-fm-inv"),  # <--
                filename="lejepa-fm-inv-{epoch:03d}",
                save_top_k=-1,
                every_n_epochs=300,
                save_last=True,
            ),
