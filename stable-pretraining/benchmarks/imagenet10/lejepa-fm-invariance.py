"""LeJEPA ViT-S/T on ImageNet-10 — Variant 5: FM-Invariance.

Idea: Batch-7 Idea 1 (TOY).
Replaces MSE invariance with flow-matching distributional view alignment.
SIGReg (EP test) unchanged.

Usage examples
--------------
python lejepa-fm-invariance.py --backbone vit_small_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf

python lejepa-fm-invariance.py --backbone vit_tiny_patch16_224 --max_epochs 100 \\
    --data_local_path /kaggle/input/imagenet10-hf
"""
...
def get_args():
    p = argparse.ArgumentParser(description="LeJEPA + FM-Invariance on ImageNet-10")
...
def main():
    ...
    run_name = args.run_name or f"lejepa-fm-inv-{btag}-inet10"
    ckpt_dir = args.checkpoint_dir or str(Path(__file__).parent / "checkpoints" / f"lejepa-fm-inv-{btag}")
    ...
    model = LeJEPAFMInv(
        ...
        n_points=17, projector_dim=args.proj_dim, fm_sigma=0.01,
    )
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=ckpt_dir, filename=f"lejepa-fm-inv-{btag}-{{epoch:03d}}",
