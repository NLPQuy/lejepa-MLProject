...
# Install bundled wheels (no internet needed)
for whl in sorted(glob.glob(f"{WHEELS}/*.whl")):
    r = subprocess.run(f"pip install '{whl}' --no-deps -q", shell=True, capture_output=True, text=True)
    print(f"  {'OK' if r.returncode == 0 else 'skip'}  {whl.split('/')[-1]}")

# Add stable_pretraining to PYTHONPATH for subprocesses
